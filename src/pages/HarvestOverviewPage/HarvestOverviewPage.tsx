...
export default function HarvestOverviewPage() {
  ...
  return (
    <div className="page">
      ...
      {zones.map((zone) => {
        const records = appData.harvestRecords
          .filter((item) => sheetInfo(item.managementSheetId)?.zoneId === zone.id)
          .sort((a, b) => b.harvestDate.localeCompare(a.harvestDate) || b.createdAt.localeCompare(a.createdAt));
        ...
        return (
          <section className="panel zone-overview" key={zone.id}>
            ...
            {records.length === 0 && <p className="empty-text">등록된 수확기록이 없습니다.</p>}
          </section>
        );
      })}
    </div>
