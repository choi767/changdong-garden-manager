...
창동 틀밭을 Zone, 틀, 관리그룹, 관리표 단위로 관리하는 로컬 반응형 웹 애플리케이션입니다. V1.4 코드를 기반으로 루트 프로젝트를 V2.0 시작점으로 구성했습니다.
...
V2.0 시작점은 V1.4의 배치 이미지와 좌표 구조를 사용합니다. 각 틀 좌표는 배경 기준 퍼센트 값으로 저장되며 설정 화면에서 `positionX`, `positionY`, `width`, `height`, `rotation`을 직접 편집할 수 있습니다. 향후 실제 배치 이미지 파일이 제공되면 저장소에 배경 이미지를 넣고 기존 퍼센트 좌표를 유지하는 방식으로 교체할 수 있습니다.
...
- 사진 업로드 데이터 모델은 준비되어 있으나 V2.0 시작점에서는 V1.4의 UI를 기준으로 확장합니다.
